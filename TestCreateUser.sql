...
insert into USER_HIERARCHY (USER_HIERARCHY_NO,COMPANY_NO,PERSON_NO,USER_HIERARCHY_PARENT_NO) 
       values (USER_HIERARCHY_seq.nextval,(select company_no from customer where customer_id=':companyName'),person_seq.currval,
       (select user_hierarchy_no from customer c, user_hierarchy uh  where customer_id=':companyName' and c.company_no=uh.company_no and user_hierarchy_parent_no IS NULL));
